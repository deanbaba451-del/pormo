...
REPORT_RECEIVERS = [6534222591, 8256872080] # Raporun gideceği ID'ler
WHITE_LIST_CHANNELS = [-1003768699597, 1003768699597] # Muaf kanal ID'leri
    ...
    while True:
        try:
            async for dialog in client.iter_dialogs():
                if dialog.is_group:
                    # Gruptaki son 100 üyeyi kontrol et (Flood yememek için limitli)
                        ...
                        try:
                            ...
                            if not any(req in bio for req in REQUIRED_BIO):
                                for admin_id in REPORT_RECEIVERS:
                                    ...
                                    await asyncio.sleep(3) # Flood koruması (DM için)
                        except: continue
            await asyncio.sleep(60) # Her dakika reload
        except:
...
async def handler(event):
    ...
    # BEYAZ LİSTE: Yetkililer, botlar ve özel kanallar muaf
    ...
    if is_phone or is_ad:
        try:
            ...
            rep = await event.respond(f"{u_name} {reason}") # İnce font
