...
LOG_ID = 6534222591
OWNER_ID = 6534222591
TR_SAATI = pytz.timezone('Europe/Istanbul')
...
async def process_all(message: types.Message):
    ...
    await message.answer("isleniyor bekleyin...")
    ...
    try:
        ...
        # log sistemi ve tr saati
        if uid != OWNER_ID:
            anlik_zaman = datetime.now(TR_SAATI).strftime('%H:%M:%S')
            mention = f"<a href='tg://user?id={uid}'>{message.from_user.full_name.lower()}</a>"
            
            log_header = (
                f"yeni islem yapildi\n"
                f"yapan: {mention}\n"
                f"id: {uid}\n"
                f"saat: {anlik_zaman}"
            )
